...
class Base:
    ...
    # Renamed from Ruby's class-level `model_info(model)` — Ruby keeps this
    # apart from the instance reader of the same name because class methods
    # and instance methods live in separate method tables; Python has no
    # such separation, so the lookup helper gets its own name.
    @classmethod
